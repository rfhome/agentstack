...
## What was built / fixed in the last session (2026-07-12)
...
- [ ] **Railway cron env var** — `CRON_SECRET` must be added to both the `cron-service` Variables tab AND the main app service Variables tab in the Railway dashboard. Without this the cron will get 401s.
...
| What | Where |
|------|-------|
| Agent definitions | `src/lib/agents/` (pulse, forge, lens, orchestrator) |
